...
export default function Team4Col({ data }: CommonSection) {
  return (
    <div className="bg-white py-10 md:py-6 lg:py-10 xl:py-[60px] 3xl:py-[80px]">
      <div className="container grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 xl:gap-4 4xl:gap-x-4 4xl:gap-y-6 xl:grid-cols-6">
        ...
        {data?.doctors?.map((item: any, index: number) => (
          <div
            key={index}
            className={`${index === 0 ? 'col-span-full flex justify-center xl:justify-end xl:col-span-2 xl:pl-[100px]' : ''}`}
          >
            <div className={`${index === 0 ? "w-1/2 md:w-[calc((100%-24px*2)/3)] xl:w-full" : ""}`}>
              <DoctorCard key={index} item={item} type="uniform_logo" />
            </div>
          </div>
        ))}
      </div>
