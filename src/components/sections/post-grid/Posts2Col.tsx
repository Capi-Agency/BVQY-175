              ...
              {items?.length > 0 &&
                items?.map((item: any, index: number) => (
                  <SwiperSlide key={'post_' + index}>
                    <Link
                      href={getAssetUrlById(item?.cover?.id || item?.cover)}
                      data-fancybox="gallery"
                      className="group relative block space-y-4 bg-primary-50 p-3 text-start transition-all duration-200 hover:bg-primary-600 xl:p-4"
                    >
                      {/* cover */}
